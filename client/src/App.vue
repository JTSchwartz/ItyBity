<template>
  <v-app>
    <Logo id="logo" :style="{fill: $vuetify.theme.themes['light'].accent}"/>
    <Intro/>
    <br/>
    <v-container fluid>
      ...
      this.apiRequest("create", {
        ...
        "slug": this.create.slug
          ...
          .then(data => {
            ...
            const qrUrl = `http://qr.itybity.xyz/?size=${this.qrSize}&url=${this.create.dest}`
            fetch(qrUrl, {
              mode : "no-cors",
              method: "GET",
              headers: {
